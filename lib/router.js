...
module.exports = app => {
  ...
  routes.get('/installations', async (req, res) => {
    ...
    res.json({ repos, installs: reposToInstalls(repos), limiter: app.limiter.counts() })
  })
  ...
  routes.get('/badge/:type', async (req, res) => {
    ...
    try {
      switch (req.params.type) {
        case 'installed':
          const installCount = Object.keys(reposToInstalls(app.scheduler.repos)).length
          ...
          break
        case 'managing':
          const repoCount = Object.keys(app.scheduler.repos).length
          params = ('plain' in req.query) ? {
            subject: '',
            status: `${repoCount}`,
            color: 'blue'
  ...
  routes.get('/check/:owner/:repo', (req, res) => {
    ...
    getJSON(`https://api.github.com/repos/${req.params.owner}/${req.params.repo}/contents/.github/${app.CONFIG_FILENAME}?${GITHUB_CREDENTIALS}`)
      ...
      .then((config) => {
        ...
        const reqs = value.rules.map(r => new Promise((resolve, reject) => {
          getJSON(`https://api.github.com/repos/${req.params.owner}/${req.params.repo}/compare/${r.base}...${r.upstream}?${GITHUB_CREDENTIALS}`)
      ...
      .catch(e => {
        let output = { statusCode: 400, body: `.github/${app.CONFIG_FILENAME} file has failed validation` }
        ...
        app.log.debug(e, `[${req.params.owner}/${req.params.repo}] [.github/${app.CONFIG_FILENAME}] ${output.message}`)
        res.status(output.statusCode).end(output.body)
      })
  })

  function reposToInstalls (repos = {}) {
    const installs = {}
    for (const repo in repos) {
      installs[repo.split('/')[0]] = (installs[repo.split('/')[0]] || 0) + 1
    }
    return installs
  }
}
