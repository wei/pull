...
module.exports = class SyncUp {
  ...
  async routineCheck () {
    ...
    const refs = Object.keys(this.config.rules)
    for (let i = 0; i < refs.length; i++) {
      const ref = refs[i]
      const c = this.config.rules[ref]

      if (ref.toLowerCase() === c.upstream.toLowerCase().replace(`${this.config.owner.toLowerCase()}:`, '')) {
        this.logger.debug(`[${this.config.repoPath}] ${ref} is same as ${c.upstream}`)
      } else if (!(await this.hasDiff(ref, c.upstream))) {
        this.logger.info(`[${this.config.repoPath}] ${ref} is in sync with ${c.upstream}`)
      } else if (await this.hasOpenPR(ref)) {
        this.logger.info(`[${this.config.repoPath}] Already has PR from ${c.upstream} to ${ref}`)
      } else {
        this.logger.info(`[${this.config.repoPath}] Creating PR from ${c.upstream} to ${ref}`)
        await this.createPR(ref, c.upstream)
      }
    }
  }
  ...
  async checkAutoMerge (incomingPR, config = {}) {
    ...
    const c = this.config.rules[incomingPR.base.ref]

    if (c.autoMerge &&
      incomingPR.state === 'open' &&
      incomingPR.user.login === 'sync-up[bot]' &&
      incomingPR.mergeable !== false
    ) {
      const mergable = incomingPR.mergeable_state === 'clean' || await this.isMergeable(prNumber, { maxRetries: config.isMergableMaxRetries || undefined })
      this.logger.debug(`[${this.config.repoPath}]#${prNumber} Mergability ${mergable}`)
      if (!mergable) {
        // Do nothing
      } else {
        await this.mergePR(prNumber, c.autoMergeMethod)
        this.logger.debug(`[${this.config.repoPath}]#${prNumber} Auto merged pull request`)

        if (c.autoMergeHardReset) {
  ...
  async hasOpenPR (ref) {
    const res = await this.github.search.issues({
      q: `repo:${this.config.repoPath} type:pr is:open base:${ref} author:app/sync-up`,
  ...
  async createPR (ref, upstream) {
    const createdPR = await this.github.pullRequests.create({
      ...
      title: helper.getPRTitle(ref, upstream),
    ...
    await this.addReviewers(prNumber, this.config.reviewers)
