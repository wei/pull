...
module.exports = class Pull {
  ...
  async checkAutoMerge (incomingPR, config = {}) {
    ...
    if (rule && 
      ...
      if (rule.autoMergeRebase &&
        incomingPR.mergeable !== true
      ) {
          await.this.mergeRebasePR(prNumber)
          this.logger.info(`[${this.config.repoPath}]#${prNumber} Rebase merged pull request`)
          return true
      }
    } else {
  ...
  async hardResetCommit (baseRef, sha) {
    ...
    return this.github.gitdata.updateRef({
      owner: this.config.owner,
      repo: this.config.repo,
      ref: `heads/${baseRef}`,
      sha,
