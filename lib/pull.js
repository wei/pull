  ...
  async hasDiff (base, upstream) {
    try {
      ...
      if (ex.message.match(/this diff is taking too long to generate/i)) {
        ...
        this.logger.debug(`[${this.config.repoPath}] ${this.config.owner}:${this.config.base}...${upstream} Not found`)
        return false
      } else if (ex.message.match(/no common ancestor/i)) {
        this.logger.debug(`[${this.config.repoPath}] ${this.config.owner}:${this.config.base}...${upstream} No common ancestor`)
        return false
      } else {
        this.logger.warn({
          err: ex,
