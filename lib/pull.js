...
module.exports = class Pull {
  constructor (github, { owner, repo, logger = console }, config) {
    ...
    if (error) {
      this.logger.warn({ e: new Error(error), owner, repo }, 'Invalid config')
  ...
  async checkAutoMerge (incomingPR, config = {}) {
    ...
    if (incomingPR.mergeable === false) {
      this.logger.debug(`ConflictReviewers : ${rule.conflictReviewers} assigned to pull request due to merge conflict`)
      ...
      try {
        ...
        await this.addLabel('merge-conflict', 'ff0000', 'Resolve conflicts manually')
      }
      ...
      await this.github.issues.update({
        ...
        labels: [this.config.label, 'merge-conflict'],
        body: helper.getPRBody(this.config.repoPath, prNumber)
      })

      await this.addReviewers(prNumber, rule.conflictReviewers)
    } else { this.logger.debug(`[${this.config.repoPath}]#${prNumber} Pull request is mergeable , no need of conflict reviewers`) }
      ...
      if (mergeableStatus && mergeableStatus.mergeable) {
        if (rule.mergeMethod === 'hardreset') {
          ...
          try {
            ...
            this.logger.warn({ e }, `[${this.config.repoPath}]#${prNumber} Hard reset failed`)
          }
        } else {
  ...
  async createPR (base, upstream, assignees, reviewers) {
    try {
      ...
      this.logger.warn(e, `[${this.config.repoPath}] Create PR from ${upstream} failed`)
      return null
    }
  }
